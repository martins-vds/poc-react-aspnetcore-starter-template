...
export const parseChallenges = (header: string) => {
    ...
    const challengeMap: { [key: string]: string } = {}; // Add type annotation for challengeMap
 ...
 */
const populateClaim = (claim: string, value: string | number, description: string, index:number, claimsObject: (string|number)[][]) => {
    // eslint-disable-next-line prefer-const
    let claimsArray: (string|number)[] = [];
    claimsArray[0] = claim;
    claimsArray[1] = value;
